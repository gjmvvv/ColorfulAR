...
namespace GJM.Helper
{
    ...
    public class ConfigReader
    {
        ...
        public ConfigReader(byte[] bytes) { mBuffer = bytes; }
        public ConfigReader(TextAsset asset) { mBuffer = asset.bytes; }
        ...
        public string ReadLine()
        {
            ...
            if (end < max)
            {
                for (; ; )
                {
                    if (end < max) { int ch = mBuffer[end++]; if (ch != '\n' && ch != '\r')                                        continue; }
                    else ++end;
        ...
        public Dictionary<string, Dictionary<string, string>> ReadDictionary()
        {
            ...
            while (canRead)
            {
                ...
                if (line.StartsWith("["))
                {
                    ...
                    if (!dict.ContainsKey(mainKey)) dict.Add(mainKey, new Dictionary<string, string>());
                }
                else
                {
                    ...
                    if (split.Length == 2) { string key = split[0].Trim(); string val = split[1].Trim(); dict[mainKey].Add(key, val); }
                }
            }
            /*
            Debug.Log("key");
